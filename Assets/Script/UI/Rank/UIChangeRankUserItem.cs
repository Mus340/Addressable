...
public class UIChangeRankUserItem : MonoBehaviour, IDynamicScrollViewItem
{
    ...
    public void onUpdateItem(int index)
    {
        ...
        if (index == UIMain.Ins.UiPopup.GetPopup<UIChangeRankPopup>(PopupType.ChangeRank).curRank)
        {
            bgImage.color = new Color(0.5843138f, 0.2588235f, 1);
        }
        else if (index == UIMain.Ins.UiPopup.GetPopup<UIChangeRankPopup>(PopupType.ChangeRank).prevRank)
        {
            bgImage.color = new Color(0.2216981f, 0.7895969f, 1);
        }
        else
        {
